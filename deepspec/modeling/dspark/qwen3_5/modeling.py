...
def _apply_rotary_pos_emb_dspark(q, k, cos, sin, unsqueeze_dim=1):
    """
    Adapts the partial-rotation RoPE of Qwen3.5 for DSpark's cross-attention.

    q: [batch, heads, draft_len, head_dim]
    k: [batch, heads, ctx_len+draft_len, head_dim]
    cos/sin: [batch, ctx_len+draft_len, rotary_dim]  (rotary_dim < head_dim)
    """
    cos = cos.unsqueeze(unsqueeze_dim)  # [batch, 1, full_len, rotary_dim]
    sin = sin.unsqueeze(unsqueeze_dim)

    q_len = q.size(-2)  # draft_len
    rotary_dim = cos.shape[-1]

    # Query uses the last q_len positions (draft positions).
    cos_q = cos[..., -q_len:, :]
    sin_q = sin[..., -q_len:, :]
...
class Qwen3_5DSparkAttention(nn.Module):
    """DSpark cross-attention using Qwen3.5's full-attention variant (with output gate)."""
    ...
    def __init__(self, config: Qwen3_5TextConfig, layer_idx: int):
        ...
        # q_proj is doubled for the output gate
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Compute query + gate from draft hidden states
        ...
        # Keys and values from [context, draft]
        ...
        q, k = _apply_rotary_pos_emb_dspark(q, k, cos, sin)
        ...
        # Cast float mask to query dtype; BlockMask (flex_attention) is not a tensor.
        ...
        # Apply output gate (Qwen3.5 full-attention feature)
...
class Qwen3_5DSparkModel(Qwen3_5PreTrainedModel):
    ...
    # Draft model uses only full attention (no GatedDeltaNet), so flex_attention works.
